  ...
  return (
    <div className="flex flex-col min-h-screen pt-24 bg-cream">
      ...
      <section className="bg-white border-b border-charcoal/5 py-16 px-6 relative overflow-hidden">
        <div className="absolute right-0 top-0 bottom-0 w-1/3 bg-brand/5 -skew-x-12 translate-x-16" />
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-8 relative z-10">
          <div>
            <span className="text-brand uppercase tracking-[0.3em] text-xs font-bold mb-4 block">Materials & Supply</span>
          ...
          <div className="flex items-center gap-4">
             <div className="relative">
               ...
               <input type="text" placeholder="Search products..." className="pl-10 pr-4 py-3 bg-cream border border-charcoal/10 focus:outline-none focus:border-brand w-64 text-sm font-medium" />
             </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 px-6 flex-1 max-w-7xl mx-auto w-full flex flex-col md:flex-row gap-12">
        ...
        <aside className="w-full md:w-64 shrink-0">
          <div className="flex items-center gap-2 font-serif text-xl text-brand mb-6 border-b border-charcoal/10 pb-4">
            ...
            {categories.map(cat => (
              <li key={cat}>
                <button 
                  ...
                  className={`text-sm uppercase tracking-widest w-full text-left py-2 px-3 transition-colors ${activeCategory === cat ? 'bg-charcoal text-brand font-bold' : 'text-charcoal/60 hover:text-brand hover:bg-brand/5'}`}
                >
                  {cat}
                </button>
              </li>
             ...
             {displayProducts.map(product => (
               <Link to={`/products/${product.id}`} key={product.id} className="group bg-white p-4 border border-charcoal/5 shadow-sm hover:shadow-xl transition-all">
                  <div className="relative aspect-square overflow-hidden mb-4 bg-cream">
                    ...
                    <div className="absolute top-4 right-4 bg-charcoal/90 p-2 rounded-full opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all text-brand">
                      <ShoppingBag className="w-4 h-4" />
                    </div>
                  </div>
                  <h3 className="font-serif text-xl text-charcoal group-hover:text-brand transition-colors mb-2">{product.title}</h3>
